...
import scalax.collection.generic.{
  AbstractDiHyperEdge, ExtendedKey, ExtendedKeyBySingleLabel, LDiHyperEdgeToString, MultiLEdgeToString, SingleLabel
}
...
/** Template for an `implicit class` that defines the infix constructor `++` to pass a label like
  ...
  */
abstract class LDiHyperEdgeInfixConstructor[N, L, CC[X] <: AbstractDiHyperEdge[X] with ExtendedKey](
