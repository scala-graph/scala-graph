 ...
 */
case class EdgeArityRange(min: Int, max: Int, uniform: Boolean = true)
    extends RandomRange(min, max, uniform) {
  require(min >= 2)
}
 ...
 */
case class NodeDegreeRange(min: Int, max: Int, uniform: Boolean = true)
    extends RandomRange(min, max, uniform) {
  require(min >= 0)
}

case class MultiplicityRange(min: Int, max: Int, uniform: Boolean = true)
    extends RandomRange(min, max, uniform) {
  require(min >= 1)
}
