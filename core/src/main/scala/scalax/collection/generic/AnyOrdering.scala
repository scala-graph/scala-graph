...
class AnyOrdering[N] extends Ordering[N]{
  ...
  protected def typeOf(x: Any) = x match {
    ...
    case _: Char |
         _: Unit |
         _: Boolean |
         _: String => StringType(x.toString) 
  ...
  def compare(a: N, b: N): Int = (typeOf(a), typeOf(b)) match {
    ...
    case (IntegerType(_),  StringType(_)) |
         (IntegerType(_),     RefType(_)) => -1

    case (FloatType(a),   FloatType(b)) => a.compare(b.self)
    case (FloatType(a), IntegerType(b)) => a.compare(b.self)
    case (FloatType(_),  StringType(_)) |
         (FloatType(_),     RefType(_)) => -1

    case (StringType(a),  StringType(b)) => a.compare(b.self)
    case (StringType(_), IntegerType(_)) |
         (StringType(_),   FloatType(_)) =>  1
    ...
    case (RefType(a),  RefType(b)) =>
      ...
      case (a: Ordered[_], b: Ordered[_]) => 
        try 
          a.asInstanceOf[Ordered[AnyRef]].compare(
          b.asInstanceOf[AnyRef with Ordered[AnyRef]])
        catch {
          case e: ClassCastException => fallback
        }
      case _ => fallback 
    }
    case (RefType(_), IntegerType(_)) |
         (RefType(_),   FloatType(_)) |
         (RefType(_),  StringType(_)) => 1
  }
}