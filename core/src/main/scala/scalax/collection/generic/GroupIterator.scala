...
trait GroupIterator[A] extends Iterator[A]
{
   ...
   * country iterator. */
  trait OutermostIterator[A]
    extends LevelIterator[A]
    with    OutermostIteratorDecl
  {
   ...
   * grouped by countries, the implementation of this trait will correspond to the
   * city iterator. */
  trait InnermostIterator[A]
    extends LevelIterator[A]
    with    InnermostIteratorDecl
  {
    protected[GroupIterator] final def init {
