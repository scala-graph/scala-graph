  ...
  with    Serializable
{
  ...
  override def clone = (newNonCheckingBuilder ++= (if (isHash) hashSet else arr)) result
  ...
  protected final def resizeArray(fromCapacity: Int, toCapacity: Int) {
  ...
  protected final def setToArray(set: Iterable[A], size: Int) {
