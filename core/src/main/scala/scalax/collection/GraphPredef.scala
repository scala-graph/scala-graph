...
import GraphEdge.{AbstractDiEdge, AbstractDiHyperEdge, EdgeCopy, EdgeLike}
...
object GraphPredef {
  ...
  /** Denotes all directed edge types for the `E` type parameter of a `Graph`.
    ...
    */
  type DiHyperEdgeLikeIn[+N] = AbstractDiHyperEdge[N]
    with EdgeCopy[AbstractDiHyperEdge]
    with OuterEdge[N, AbstractDiHyperEdge]
