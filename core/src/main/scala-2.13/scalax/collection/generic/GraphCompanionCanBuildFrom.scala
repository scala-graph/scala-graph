...
private[collection] trait GraphCompanionCanBuildFrom[
  ...
  class GraphCanBuildFrom[N, E[+X] <: EdgeLikeIn[X]](implicit edgeT: ClassTag[E[N]], config: Config)
    ...
    def fromSpecific(from: Coll @uncheckedVariance)(it: IterableOnce[Param[N, E]]): CC[N, E] =
      newBuilder(from).addAll(it).result()

    def newBuilder(from: Coll @uncheckedVariance): mutable.Builder[Param[N, E], CC[N, E]] =
      GraphCompanionCanBuildFrom.this.newBuilder(edgeT, config)
  }
}
