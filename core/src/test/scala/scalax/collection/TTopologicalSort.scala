...
private class TTopologicalSort[G[N, E[+X] <: EdgeLikeIn[X]] <: Graph[N, E] with GraphLike[N, E, G]](
  ...
  private object Topo {
    ...
    def unexpectedCycle[N, E[+X] <: EdgeLikeIn[X]](cycleNode: Graph[N, E]#NodeT): Nothing =
      fail(s"Unexpected cycle starting at ${cycleNode.value}")

    def unexpectedRight[N, E[+X] <: EdgeLikeIn[X]](order: Graph[N, E]#TopologicalOrder[_]): Nothing =
