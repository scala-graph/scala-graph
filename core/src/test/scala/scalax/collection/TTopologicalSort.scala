...
private class TTopologicalSort[G[N,E[X] <: EdgeLikeIn[X]] <: Graph[N,E] with GraphLike[N,E,G]] 
  ...
  private object Topo {
    ...
    class Checker[N, E[X] <: EdgeLikeIn[X]](val graph: G[N,E]) {
      ...
      def checkInnerNodes(seq: OrderedInnerNodes, root: Option[graph.NodeT] = None,
      ...
      def checkCompletenis(seq: OrderedInnerNodes, maybeRoot: Option[graph.NodeT], ignorePredecessors: Boolean): Unit = {
        val expected = maybeRoot.fold(
          ifEmpty = graph.nodes.toSet
        ){ root => root.innerNodeTraverser().withParameters(Dfs(AnyConnected)).toSet --
                  (if (ignorePredecessors) predecessors(maybeRoot) else Nil)
        }
        val set = seq.toSet
        if (set != expected)
          fail(s"Ordering is incomplete when root=$maybeRoot and ignorePredecessors=$ignorePredecessors: expected ${expected} but was ${set}.")
      }
    }
