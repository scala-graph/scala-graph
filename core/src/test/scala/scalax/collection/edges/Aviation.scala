...
object Aviation {
  ...
  case class DayTime(oClock: Int, min: Int) extends HHMM(oClock, min) {
    assert(oClock < 24)
  }
  ...
  case class Duration(hour: Int, min: Int) extends HHMM(hour, min) {
    override def toString = fmt(hour) + "h" + fmt(min) + "'"
  }

  final class DurationAssoc(hours: Int) {
    def h(mins: Int) = Duration(hours, mins)
  }
