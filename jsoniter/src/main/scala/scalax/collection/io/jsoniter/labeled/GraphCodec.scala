...
object GraphCodec extends IdBasedGraphCodec:
  ...
  /** Produces a codec for a Graph with labeled edges where JSON edge ends are represented by node IDs.
    ...
    * @param toLabel function to produce the label for a given edge. Labels are typically of some primitive-like type
    *                or tuples. They will be passed to `withNodeReferences` during encoding.
    *                When working with an ADT of edges, edges types might have different labels. To deal with this
    *                situation, you need to provide an ADT for `L` and a separate `case` for every edge type accordingly.
    ...
    * @tparam L the intermediate type of edge labels to be used in `ER`.
