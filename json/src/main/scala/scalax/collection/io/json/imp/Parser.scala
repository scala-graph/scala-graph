...
sealed abstract protected[json] class JsonList(jsonValues: Iterable[JValue]) extends Iterable[JValue] {
  override def iterator: Iterator[JValue] = jsonValues.iterator
}
...
object Parser {
  ...
  def parse[N](json: String, descriptor: Descriptor[N]): Iterable[ElemList] =
    parse(JsonParser.parse(json), descriptor)

  def parse[N](jsonAST: JValue, descriptor: Descriptor[N]): Iterable[ElemList] =
    jsonAST match {
      case JObject(fields) => parse(fields, descriptor)
      case _               => Seq.empty[ElemList]
    }
