...
private class CustomEdge[CC[N, E <: Edge[N]] <: AnyGraph[N, E] with GraphLike[N, E, CC]](
  ...
  object `JSON import/export of graphs with typed edges` {
    ...
    def `on import`(): Unit =
      factory.fromJson[String, Transition](jsonText, descriptor) shouldBe graph
  ...
  object `JSON import/export of hypergraphs ` {
    object `with typed hyperedges` {
      ...
      def `on import`(): Unit =
        factory.fromJson[String, WLHyperEdge](jsonText, descriptor) shouldBe graph
    ...
    object `with typed directed hyperedges` {
      ...
      def `on import`(): Unit =
        factory.fromJson[String, WLDiHyperEdge](jsonText, descriptor) shouldBe graph
