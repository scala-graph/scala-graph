...
class TGraphComponentsTest extends FunSuite with Matchers {
  ...
  private def trace(s: String, firstN: Int): Unit = {
    s take firstN foreach (c => print(f"${c.toInt}%03d-"))
    println()
  }
}