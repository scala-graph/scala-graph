...
class ConstraintBinaryOp[N, E <: Edge[N], G <: Graph[N, E]](
  ...
  protected class PreCheckResults(
      override val followUp: PreCheckFollowUp,
  ...
  final override def preCreate(nodes: Iterable[N], edges: Iterable[E]): PreCheckResult =
  ...
  final override def preAdd(edge: E): PreCheckResult = eval(left, left preAdd edge, right, right preAdd edge)
  ...
  final override def preSubtract(
      nodes: => Set[self.NodeT],
  ...
  final override def postSubtract(
      newGraph: G @uV,
      passedNodes: Iterable[N],
      passedEdges: Iterable[E],
      preCheck: PreCheckResult
