...
class Acyclic[N, E <: EdgeLikeIn[N], G <: Graph[N, E]](override val self: G) extends Constraint[N, E, G](self) {
  ...
  /** When inserting an edge with a source contained in the graph a cycle is produced
    ...
    */
  def preAdd(edge: E): PreCheckResult = PreCheckResult.complete(
    edge.nonLooping && {
...
object Acyclic extends ConstraintCompanion[Acyclic] {
  def apply[N, E <: EdgeLikeIn[N], G <: Graph[N, E]](self: G) = new Acyclic[N, E, G](self)
}
